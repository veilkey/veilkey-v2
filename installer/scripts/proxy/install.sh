...
mkdir -p "${LIB_DIR}" "${LIB_DIR}/snippets"
for script in verify-proxy-lxc.sh install-veilroot-boundary.sh install-veilroot-codex.sh; do
...
info ""
info "installed:"
info "  ${BIN_DIR}/veilkey-proxy"
info "  ${BIN_DIR}/veilkey-session-config"
info "  ${ETC_DIR}/session-tools.toml"
info "  ${ETC_DIR}/proxy.env"
info "  ${SYSTEMD_DIR}/veilkey-veilroot-observe@.service"
info "  ${SYSTEMD_DIR}/veilkey-veilroot-egress-guard@.service"
info ""
info "next steps:"
info "  1. edit ${ETC_DIR}/session-tools.toml for your environment"
info "  2. run install-veilroot-boundary.sh to set up veilroot user"
