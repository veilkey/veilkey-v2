...
// and returns a partially wired harness. Call finalize() after
// optionally starting an agent server.
...
// secretMap maps ref paths (e.g. "myref" or "group/key") to plaintext values.
...
func (th *testHarness) insertAgent(t *testing.T, hash, vaultName, label string, blocked bool, rebind bool) {
	...
	// Set the agent DEK and agent hash
...
func TestV1Resolve_DEKDecryptionFailure(t *testing.T) {
	...
	// Overwrite DEK with one encrypted by a different KEK
