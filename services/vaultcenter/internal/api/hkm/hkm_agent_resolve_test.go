...
// stubDeps implements hkm.Deps for handler-level testing.
...
// testHarness bundles everything needed for a handler integration test.
...
// newTestHarness creates an in-memory DB, generates crypto keys,
// and returns a fully wired Handler ready for HTTP tests.
// The caller must call h.agentSrv.Close() when done.
...
// encryptDEK encrypts the agent DEK with the KEK.
...
// encryptSecret encrypts a secret value with the agent DEK.
...
// startAgentServer creates an httptest server that responds to cipher requests.
// secretMap maps ref paths (e.g. "myref" or "group/key") to their encrypted response.
...
// finalize wires up the Handler with the stubDeps. Must be called after startAgentServer.
func (th *testHarness) finalize(t *testing.T) {
	...
	if th.agentSrv != nil {
		deps.httpClient = th.agentSrv.Client()
	}
	th.handler = NewHandler(deps)
}

// insertAgent inserts a test agent with encrypted DEK pointing to the mock agent server.
func (th *testHarness) insertAgent(t *testing.T, hash, vaultName, label string, blocked bool, rebind bool) {
	...
	// Parse agent server URL to get IP and port
	ip := "127.0.0.1"
	port := 0
	if th.agentSrv != nil {
	...
	agent := db.Agent{
		...
		DEK:            encDEK,
		DEKNonce:       encNonce,
		RebindRequired: rebind,
		Version:        1,
	}
	if blocked {
		now := time.Now()
		agent.BlockedAt = &now
		agent.BlockReason = "test-blocked"
	}

	if err := th.db.UpsertAgent(agent.NodeID, agent.Label, agent.VaultHash, agent.VaultName, agent.IP, agent.Port, 0, 0, 1, 1, ""); err != nil {
		t.Fatalf("upsert agent: %v", err)
	}
	// Update DEK and flags directly
	th.updateAgentFields(t, agent.NodeID, encDEK, encNonce, blocked, rebind, hash)
}

func (th *testHarness) updateAgentFields(t *testing.T, nodeID string, dek, dekNonce []byte, blocked, rebind bool, hash string) {
	t.Helper()
	// Use raw SQL via the DB's exposed connection if available,
	// or use the UpsertAgent + update DEK pattern.
	// Since we can't access conn directly, we'll update DEK via UpdateAgentDEK if available.
	// Let's check what methods are available...
	// We'll use the SetAgentDEK method if it exists, otherwise work around it.
	if err := th.db.SetAgentDEK(nodeID, dek, dekNonce); err != nil {
		t.Fatalf("set agent DEK: %v", err)
	}
	if blocked {
		if err := th.db.BlockAgent(nodeID, "test-blocked"); err != nil {
			t.Fatalf("block agent: %v", err)
		}
	}
	if rebind {
		if err := th.db.SetAgentRebindRequired(nodeID, true, "test-rebind"); err != nil {
			t.Fatalf("set rebind: %v", err)
		}
	}
}

// callResolve makes a GET /api/resolve-agent/{token} request and returns the response.
...
// parseResp decodes a JSON response body.
...
func TestV1Resolve_EmptyToken(t *testing.T) {
	th := newTestHarness(t)
	th.finalize(t)

	rec := th.callResolve(t, "")
	// Empty path is ambiguous; the mux may return 404 because "/" won't match the route.
	// We accept 400 or 404 here.
...
func TestV1Resolve_SecretNotFoundOnAgent(t *testing.T) {
	th := newTestHarness(t)
	th.startAgentServer(t, map[string]string{}) // empty — no secrets
	th.finalize(t)
	th.insertAgent(t, "abcd1234", "test-vault", "Test Vault", false, false)
...
func TestV1Resolve_DEKDecryptionFailure(t *testing.T) {
	...
	// Insert agent but corrupt the DEK encryption by using a different KEK
	badKEK, _ := crypto.GenerateKey()
	badEncDEK, badNonce, _ := crypto.Encrypt(badKEK, th.agentDEK)
	th.insertAgent(t, "abcd1234", "test-vault", "Test Vault", false, false)
	// Overwrite with bad DEK
	if err := th.db.SetAgentDEK("node-abcd1234", badEncDEK, badNonce); err != nil {
		t.Fatal(err)
	}
...
// ── V2: path-based resolve tests ────────────────────────────────────────────

func TestV2Resolve_HappyPath(t *testing.T) {
	th := newTestHarness(t)
	th.startAgentServer(t, map[string]string{
		"owner/api-key": "super-secret-value",
	})
	th.finalize(t)
...
func TestV2Resolve_SecretNotFound(t *testing.T) {
	th := newTestHarness(t)
	th.startAgentServer(t, map[string]string{}) // empty
	th.finalize(t)
	th.insertAgent(t, "abcd1234", "host-lv", "Host LV", false, false)
...
func TestV2Resolve_DEKDecryptionFailure(t *testing.T) {
	...
	th.db.SetAgentDEK("node-abcd1234", badEnc, badNonce)
...
func TestV2Resolve_InvalidPathEmptySegment(t *testing.T) {
	th := newTestHarness(t)
	th.finalize(t)

	rec := th.callResolve(t, "vault//key")
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected 400, got %d: %s", rec.Code, rec.Body.String())
	}
}
...
func TestSecurity_V2PathTraversal(t *testing.T) {
	...
	// Segments with ".." or "/" injection should be rejected by v2PathSegmentPattern
	cases := []string{
		"../etc/passwd",
		"vault/../secret/key",
		"vault/group/..%2f..%2fetc",
	}
...
func TestSecurity_V2UppercaseSegmentsRejected(t *testing.T) {
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 for uppercase segments, got %d: %s", rec.Code, rec.Body.String())
	}
}
...
func TestSecurity_V2SpecialCharsRejected(t *testing.T) {
	...
	badTokens := []string{
		"vault/group/key;rm -rf",
		"vault/group/key$HOME",
		"vault/group/key`id`",
		"vault/group/key{test}",
		"vault/group/key<script>",
	}
...
func TestSecurity_V1TamperedCiphertext(t *testing.T) {
	// Agent returns ciphertext encrypted with a different key — decryption should fail
	th := newTestHarness(t)
...
func TestSecurity_AgentUnreachable(t *testing.T) {
	...
	th.finalize(t)
	th.insertAgent(t, "abcd1234", "test-vault", "Test Vault", false, false)
	srv.Close() // make agent unreachable
...
// ── V1/V2 routing dispatch test ─────────────────────────────────────────────

func TestResolve_DispatchV1VsV2(t *testing.T) {
	th := newTestHarness(t)
	th.startAgentServer(t, map[string]string{
		"secret123":     "v1-value",
		"owner/api-key": "v2-value",
	})
	th.finalize(t)
	th.insertAgent(t, "abcd1234", "host-lv", "Host LV", false, false)

	// V1: no slash in token
	...
	dataV1 := parseResp(t, recV1)
	if dataV1["value"] != "v1-value" {
		t.Errorf("V1 value = %v, want v1-value", dataV1["value"])
	}
	...
	dataV2 := parseResp(t, recV2)
	if dataV2["value"] != "v2-value" {
		t.Errorf("V2 value = %v, want v2-value", dataV2["value"])
	}
}
